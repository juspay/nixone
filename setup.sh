...
if ! which nix > /dev/null; then
  # Install Nix
  curl --proto '=https' --tlsv1.2 -sSf -L https://artifacts.nixos.org/nix-installer/tag/2.34.6/nix-installer.sh | \
    sh -s -- install --no-confirm --extra-conf "trusted-users = $(whoami)" --enable-flakes
